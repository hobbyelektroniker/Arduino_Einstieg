...
void schalteLeds() {
  ...
  static unsigned long blinkStartZeit = 0;
  static bool hell = true;
  if (tic > blinkStartZeit + blinkZeit) {
    hell = !hell;
    for (int ledNummer=0; ledNummer<anzahl_leds; ledNummer++) {
      switch (ledStatus[ledNummer]) {
        case aus: digitalWrite(ledPin[ledNummer],LOW);
                  break;
        case ein: digitalWrite(ledPin[ledNummer],HIGH);
                  break;
        case blinken: digitalWrite(ledPin[ledNummer],hell);
      }
      blinkStartZeit = tic;
    }
  }  
